...
def accept_appointment(request, pk):
    ...
    try:
        ...
        messages.warning(request, f'Appointment Accepted but email failed due to SSL error. Please check your internet connection.')
    except (ConnectionError, OSError, Exception) as e:
        error_msg = str(e).lower()
        if 'getaddrinfo' in error_msg or 'network' in error_msg or 'dns' in error_msg:
            messages.warning(request, 'Appointment Accepted but email failed due to network connectivity issues. Please check your internet connection.')
        else:
            messages.warning(request, f'Appointment Accepted but email failed: Network or server issue. Please try again later.')
